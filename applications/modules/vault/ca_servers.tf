...
resource "vault_pki_secret_backend_intermediate_cert_request" "servers_intermediate" {
...
resource "vault_pki_secret_backend_root_sign_intermediate" "servers_intermediate" {
  depends_on = [vault_pki_secret_backend_intermediate_cert_request.servers_intermediate, vault_pki_secret_backend_config_ca.ca_config]
  backend    = vault_mount.root.path

  csr                  = vault_pki_secret_backend_intermediate_cert_request.servers_intermediate.csr
  common_name          = vault_pki_secret_backend_intermediate_cert_request.servers_intermediate.common_name
...
resource "vault_pki_secret_backend_intermediate_set_signed" "servers_intermediate" {
  backend     = vault_mount.pki_int.path
  certificate = "${vault_pki_secret_backend_root_sign_intermediate.servers_intermediate.certificate}\n${tls_self_signed_cert.ca_cert.cert_pem}"
}